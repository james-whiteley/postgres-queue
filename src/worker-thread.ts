  ...
  if (action === 'start_job') {
    ...
    if (!job) {
      ...
      try {
        ...
        const res = await consumer.query('UPDATE jobs SET status = $1, progress = 100, updated_at = NOW() WHERE id = $2 AND queue = $3 RETURNING *', ['completed', job.id, workerData.queueName]);
        const completedJob = res.rows[0];
        parentPort?.postMessage({ action: 'job_completed', job: completedJob, returnValue: result });
      } catch (error: any) {
        if (job.attempts === (job.retries + 1)) {
          const response = await consumer.query('UPDATE jobs SET status = $1, updated_at = NOW() WHERE id = $2 AND queue = $3 RETURNING *', ['failed', job.id, workerData.queueName]);
          const failedJob = response.rows[0];
          parentPort?.postMessage({ action: 'job_failed', job: failedJob, error: error });
        } else {
          const response = await consumer.query('UPDATE jobs SET attempts = $1, status = $2, updated_at = NOW() WHERE id = $3 AND queue = $4 RETURNING *', [Number(job.attempts) + 1, 'idle', job.id, workerData.queueName]);
          const retryJob = response.rows[0];
