...
export class Queue extends DBConnection {
  ...
  async enqueue(job: { data: object, delay?: number, retries?: number, backoff_strategy?: 'linear' | 'exponential', priority?: number }) {
    ...
    try {
      ...
      await client.query(`NOTIFY new_job_${this.queueName}`);
      return res.rows[0];
    } catch (error: unknown) {
      throw error;
    } finally {
...
export class Consumer extends DBConnection {
  ...
  async listenForJobNotifications() {
    ...
    try {
      ...
      this.jobListenerClient = await this.queryWithoutRelease(`LISTEN new_job_${this.queueName}`);
  ...
  async unlistenForJobNotifications() {
    ...
    await this.jobListenerClient?.query(`UNLISTEN new_job_${this.queueName}`);
